...
for i = 1:nt
    if hcurve(i).locscreen == 1
        hcurve(i).stsmooth = nan(length(hcurve(i).styears),1);
        hcurve(i).trsmooth = nan(length(hcurve(i).tryears),1);
    end
end

for i = 1:nt
    if hcurve(i).locscreen == 1
        nntr = sum(~isnan(hcurve(i).trtemp));
        nnst = sum(~isnan(hcurve(i).stmean));
        if (nntr > 20 && nnst > 20)
            hcurve(i).mindatsm = 1;
            
            %decadally smoothed
            indmst = isfinite(hcurve(i).stmean);
            indmtr = isfinite(hcurve(i).trtemp);
            hcurve(i).stsmooth(indmst) = hepta_smooth(hcurve(i).stmean(indmst), 1/10);
            hcurve(i).trsmooth(indmtr) = hepta_smooth(hcurve(i).trtemp(indmtr), 1/10);
...
for i = 1:nt
    ...
    if hcurve(i).timescreen == 2
        ...
        hcurve(i).trsm1(tr1ind) = hcurve(i).trsmooth(tr1ind2);
        hcurve(i).trsm2(tr2ind) = hcurve(i).trsmooth(tr2ind2);
        hcurve(i).stsm1(st1ind) = hcurve(i).stsmooth(st1ind2);
        hcurve(i).stsm2(st2ind) = hcurve(i).stsmooth(st2ind2);
        
        hcurve(i).trsm = vertcat(hcurve(i).trsm1, hcurve(i).trsm2);
        hcurve(i).stsm = vertcat(hcurve(i).stsm1, hcurve(i).stsm2);

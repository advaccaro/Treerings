...
icon{1,1}= [0 .4 0];   icon{1,2}= '^';
icon{1,3} = sprintf('MXD Data, n = %u', ng1); 
icon{1,4} = 'MXD Data';
...
m_grid('xtick',[0:60:360],'tickdir','out','ytick',[-90:30:90], 'color','r', 'fontsize',10,'fontname','Times New Roman');
...
[LEGH,OBJH]=legend(hl(:),icon{1,3});%pause; 
...
title('Global Tree-Ring MXD Availability','FontWeight','bold','FontSize',14,'FontName','Times');
...
fancyplot_deco('Tree-Ring MXD availability in time','Time','# proxies');

[LEGH,OBJH,OUTH,OUTM]=legend(hb,icon{1,4},'Location','NorthWest'); % 4 is for Lower right-hand corner
...
print -painters -dpdf -r600 tr_allmap1.pdf

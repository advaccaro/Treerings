...
%         tcurve(j).timescreen = 0;
    if tcurve(j).locscreen == 0
        ...
        if tcurve(j).timescreen == 2
            ...
            st1ind = ismember(tcurve(j).yearsp1, tcurve(j).styrs);
            st2ind = ismember(tcurve(j).yearsp2, tcurve(j).styrs);
            
            tr1ind2 = ismember(tcurve(j).tryears, tcurve(j).yearsp1);
            tr2ind2 = ismember(tcurve(j).tryears, tcurve(j).yearsp2);
            st1ind2 = ismember(tcurve(j).styrs, tcurve(j).yearsp1);
            st2ind2 = ismember(tcurve(j).styrs, tcurve(j).yearsp2);
            ...
            tcurve(j).stdatp1(st1ind) = tcurve(j).stmean(st1ind2);
            tcurve(j).stdatp2(st2ind) = tcurve(j).stmean(st2ind2);
